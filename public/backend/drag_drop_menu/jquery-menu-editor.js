...
function MenuEditor(idSelector, options) {
    ...
    function TButtonGroup(menupage_id = '') {
        var $divbtn = $('<div menupage_id="'+menupage_id+'">').addClass('btn-group float-right');
    ...
    function createMenu(arrayItem, depth) {
        ...
        $.each(arrayItem, function (k, v) { 
            ...
            var $li = $('<li>').addClass('list-group-item pr-0 sortableListsOpen');
            $li.data(itemObject);
            var $div = $('<div>').css('overflow', 'auto');
            var $i = $('<i>').addClass(v.icon);
			var status_text = '<span class="status_text'+v.id+'"><i class="fas fa-toggle-off"></i></span>';
			if(v.status == 1){
				status_text = '<span class="status_text'+v.id+'"><i class="fas fa-toggle-on"></i></span>';
			}
			
			var newtab_url_text = '<span class="newtab_url_text'+v.id+'"></span>';
			if(v.new_tab == 1){
				newtab_url_text = '<span class="newtab_url_text'+v.id+'"><i class="fas fa-external-link-alt"></i></span>';
			}
			
			

            var $span = $('<span>').addClass('txt').append('<span class="menutext'+v.id+'">'+v.text +'</span> <span class="menu_page_type">('+v.page_type +')</span> '+status_text+' '+newtab_url_text).css('margin-right', '5px');
            var $divbtn =  TButtonGroup(v.id);
