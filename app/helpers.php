...
function front_mobile_menu($MenuPagemodel, $menu_id, $classes = '') {

	$html = '';

	$main_ul_class = isset($classes['main_ul_class']) ? $classes['main_ul_class'] : '';

	$html .= '<div class="mobile-navigation"> <nav> <ul class="mobile-menu">';

	$html .= getMobileMenusHtml($MenuPagemodel, $menu_id, 0, 0, '', $classes);
...
function getMobileMenusHtml($MenuPagemodel, $menu_id, $parent_id = 0, $lavel, $html='', $classes = '') {
		...
		if(!empty($records)) {
			...
			foreach($records as $record) {
				if ($record['mega_menu'] == 0) {
					...
					$menusArr['children'] = getChildMobileMenusList($MenuPagemodel, $menu_id, $record['id']);
					...
					if(!empty($menusArr['children'])) {
						$child_ul_class =  isset($classes['child_ul_class']) ? $classes['child_ul_class'] : '';
						$main_li_class= isset($classes['main_li_class']) ? 'menu-item-has-children' : '';
					...
					if(!empty($menusArr['children'])) { 
						...
						$html .= getMobileMenusHtml($MenuPagemodel, $menu_id, $record['id'], $lavel);
...
function getChildMobileMenusList($MenuPagemodel, $menu_id, $parent_id = 0) {
